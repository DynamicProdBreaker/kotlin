...
class WasmLoweringFacade(
    ...
    override fun transform(module: TestModule, inputArtifact: IrBackendInput): BinaryArtifacts.Wasm? {
        ...
        val (noDceParameters, withDceParameters) = configuration.perfManager.tryMeasurePhaseTime(PhaseType.Backend) {
            configuration.dce = false
            val noDce = compiler.compileIr(loweredIr)

            configuration.dce = true
            val withDce = compiler.compileIr(loweredIr)
