...
sealed class BtaCompilerArgument<T : BtaCompilerArgumentValueType>(
    ...
    class SSoTCompilerArgumentCompat(
        ...
        constructor(origin: KotlinCompilerArgument) : this(
            SSoTCompilerArgument(origin),
        )
...
object CustomCompilerArguments {
    ...
    /**
     * Provides forward compatibility for the 'classpath' argument until Kotlin 2.5.
     * This bridge is required to support BTA-api 2.3.x consumers interacting with BTA-impl 2.4.x.
     */
    val classpathCompat = BtaCompilerArgument.CustomCompilerArgument(
        name = "classpath",
        description = "List of directories and JAR/ZIP archives to search for user class files.",
        valueType = BtaCompilerArgumentValueType.CustomArgumentValueType(
            type = listTypeNameOf(Path::class.asTypeName()).copy(nullable = true),
        ),
