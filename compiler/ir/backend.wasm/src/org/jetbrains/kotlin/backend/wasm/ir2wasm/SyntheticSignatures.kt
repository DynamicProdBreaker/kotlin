...
object Synthetics {
    ...
    private val tryGetAssociatedObjectSignature =
        IdSignature.CommonSignature(syntheticFqName, "tryGetAssociatedObjectAndWrapper", null, 0, null)
    ...
    object HeapTypes {
        val wasmAnyArrayType = WasmImmediate.HeapType(WasmHeapType.Type.GcType(wasmAnyArrayTypeSignature))
        val specialSlotITableType = WasmImmediate.HeapType(WasmHeapType.Type.GcType(specialSlotITableTypeSignature))
        val rttiType = WasmImmediate.HeapType(WasmHeapType.Type.GcType(rttiTypeSignature))
        val wasmLongArray = WasmImmediate.HeapType(WasmHeapType.Type.GcType(wasmLongArraySignature))
        val wasmLongArrayDeclaration = WasmImmediate.HeapType(WasmHeapType.Type.GcType(wasmLongArrayDeclarationSignature))
        val wasmStringArrayType = WasmImmediate.HeapType(WasmHeapType.Type.GcType(wasmStringArrayTypeSignature))
        val byteArray = WasmImmediate.HeapType(WasmHeapType.Type.GcType(byteArraySignature))
        val associatedObjectGetterType = WasmImmediate.HeapType(WasmHeapType.Type.GcType(associatedObjectGetterTypeSignature))
    }
    ...
    object FunctionGcTypes {
        val associatedObjectGetterType = WasmImmediate.TypeIdx.FunctionTypeIdx(associatedObjectGetterTypeSignature)
        val stringLiteralFunctionType = WasmImmediate.TypeIdx.FunctionTypeIdx(stringLiteralFunctionTypeSignature)
        val parameterlessNoReturnFunctionType = WasmImmediate.TypeIdx.FunctionTypeIdx(parameterlessNoReturnFunctionTypeSignature)
    }

    object FunctionHeapTypes {
        val stringLiteralFunctionType = WasmImmediate.HeapType(WasmHeapType.Type.FunctionType(stringLiteralFunctionTypeSignature))
    }
