...
annotation class DelicateCliApi(val message: String)

@DelicateCliApi("Use this overload only in case the instance of `CliDiagnosticReporter` is unavailable")
fun MessageCollector.reportArgumentParseProblems(arguments: CommonToolArguments) {
    ...
    val configuration = CompilerConfiguration().also {
        it.messageCollector = this
    }
    val cliDiagnosticReporter = CliDiagnosticReporter(diagnosticReporter, configuration)
    cliDiagnosticReporter.reportArgumentParseProblems(arguments)
    diagnosticReporter.reportToMessageCollector(this, renderDiagnosticName = false)
}
